...
makeSuite("AToken: Modifiers", (testEnv: TestEnv) => {
  ...
  it("Tries to invoke mint not being the LendingPool", async () => {
    ...
    await expect(aDai.mint(deployer.address, "1", "1")).to.be.revertedWith(CT_CALLER_MUST_BE_LENDING_POOL);
  });
  ...
  it("Tries to invoke transferOnLiquidation not being the LendingPool", async () => {
    ...
    await expect(aDai.transferOnLiquidation(deployer.address, users[0].address, "1")).to.be.revertedWith(
      CT_CALLER_MUST_BE_LENDING_POOL,
    );
  });
