...
export const borrow = async (
  ...
  if (expectedResult === "success") {
    const txResult = await waitForTx(
      ...
      await pool.connect(user.signer).borrow(reserve, amountToBorrow, onBehalfOf),
    );
    ...
    /* await expect(pool.connect(user.signer).borrow(reserve, amountToBorrow, interestRateMode, onBehalfOf), revertMessage)
      .to.be.reverted; */
    await expect(pool.connect(user.signer).borrow(reserve, amountToBorrow, onBehalfOf), revertMessage).to.be.reverted;
  }
};
...
export const repay = async (
  ...
  if (expectedResult === "success") {
    const txResult = await waitForTx(
      await pool.connect(user.signer).repay(reserve, amountToRepay, rateMode, onBehalfOf.address, txOptions),
    );
    ...
    await expect(
      pool.connect(user.signer).repay(reserve, amountToRepay, rateMode, onBehalfOf.address, txOptions),
      revertMessage,
    ).to.be.reverted;
  }
};
...
export const setUseAsCollateral = async (
  ...
  if (expectedResult === "success") {
    ...
    await expect(pool.connect(user.signer).setUserUseReserveAsCollateral(reserve, useAsCollateralBool), revertMessage)
      .to.be.reverted;
  }
};
...
export const swapBorrowRateMode = async (
  ...
  if (expectedResult === "success") {
    ...
    await expect(pool.connect(user.signer).swapBorrowRateMode(reserve, rateMode), revertMessage).to.be.reverted;
  }
};
...
export const rebalanceStableBorrowRate = async (
  ...
  if (expectedResult === "success") {
    ...
    await expect(pool.connect(user.signer).rebalanceStableBorrowRate(reserve, target.address), revertMessage).to.be
      .reverted;
  }
};
