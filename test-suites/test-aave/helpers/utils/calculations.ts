...
export const calcExpectedUserDataAfterWithdraw = (
  ...
  const aTokenBalance = calcExpectedATokenBalance(reserveDataBeforeAction, userDataBeforeAction, txTimestamp);
...
export const calcExpectedReserveDataAfterWithdraw = (
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = calcExpectedATokenBalance(reserveDataBeforeAction, userDataBeforeAction, txTimestamp).toFixed();
  }
...
export const calcExpectedReserveDataAfterBorrow = (
  ...
  if (borrowRateMode == RateMode.Stable) {
    ...
    const utilizationRateAfterTx = calcExpectedUtilizationRate(
      ...
      expectedReserveData.availableLiquidity.plus(totalStableDebtAfterTx).plus(totalVariableDebtAfterTx),
    );
...
export const calcExpectedReserveDataAfterRepay = (
  ...
  if (borrowRateMode == RateMode.Stable) {
    ...
    expectedReserveData.principalStableDebt = expectedReserveData.totalStableDebt = expectedDebt.minus(amountRepaidBN);
...
export const calcExpectedUserDataAfterRepay = (
  ...
  if (rateMode == RateMode.Stable) {
    ...
    expectedUserData.principalStableDebt = expectedUserData.currentStableDebt = stableDebt.minus(totalRepaidBN);
...
export const calcExpectedReserveDataAfterSwapRateMode = (
  ...
  const variableDebt = calcExpectedVariableDebtTokenBalance(reserveDataBeforeAction, userDataBeforeAction, txTimestamp);
...
const calcExpectedScaledATokenBalance = (
  ...
  return userDataBeforeAction.scaledATokenBalance.plus(amountAdded.rayDiv(index)).minus(amountTaken.rayDiv(index));
};
...
export const calcExpectedInterestRates = (
  ...
  if (utilizationRate.gt(optimalRate)) {
    ...
    stableBorrowRate = stableBorrowRate
      ...
      .plus(new BigNumber(reserveConfiguration.strategy.stableRateSlope2).rayMul(excessUtilizationRateRatio));
...
export const calcExpectedOverallBorrowRate = (
  ...
  const overallBorrowRate = weightedVariableRate.plus(weightedStableRate).rayDiv(totalBorrows.wadToRay());
...
const calcExpectedUserStableRate = (
  ...
  return balanceBefore.times(rateBefore).plus(amount.times(rateNew)).div(balanceBefore.plus(amount));
};
