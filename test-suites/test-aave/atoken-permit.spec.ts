...
makeSuite("AToken: Permit", (testEnv: TestEnv) => {
  ...
  it("Reverts submitting a permit with 0 expiration", async () => {
    ...
    await expect(
      aDai.connect(spender.signer).permit(owner.address, spender.address, permitAmount, expiration, v, r, s),
  ...
  it("Submits a permit with maximum expiration length", async () => {
    ...
    await waitForTx(
      await aDai.connect(spender.signer).permit(owner.address, spender.address, permitAmount, deadline, v, r, s),
    );
  ...
  it("Tries to submit a permit with invalid nonce", async () => {
    ...
    await expect(
      aDai.connect(spender.signer).permit(owner.address, spender.address, permitAmount, deadline, v, r, s),
  ...
  it("Tries to submit a permit with invalid expiration (previous to the current block)", async () => {
    ...
    await expect(
      aDai.connect(spender.signer).permit(owner.address, spender.address, expiration, permitAmount, v, r, s),
    ).to.be.revertedWith("INVALID_EXPIRATION");
  });
  ...
  it("Tries to submit a permit with invalid signature", async () => {
    ...
    await expect(
      aDai.connect(spender.signer).permit(owner.address, ZERO_ADDRESS, permitAmount, deadline, v, r, s),
    ).to.be.revertedWith("INVALID_SIGNATURE");
  });
  ...
  it("Tries to submit a permit with invalid owner", async () => {
    ...
    await expect(
      aDai.connect(spender.signer).permit(ZERO_ADDRESS, spender.address, expiration, permitAmount, v, r, s),
