  ...
  describe("BaseUniswapAdapter", () => {
    describe("getAmountsOut", () => {
      it("should return the estimated amountOut and prices for the asset swap", async () => {
        ...
        const ethUsdValue = amountIn.mul(wethPrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));
        const daiUsdValue = expectedDaiAmount.mul(daiPrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));

        await mockUniswapRouter.setAmountOut(amountToSwap, weth.address, dai.address, expectedDaiAmount);

        const result = await uniswapLiquiditySwapAdapter.getAmountsOut(amountIn, weth.address, dai.address);
      ...
      it("should work correctly with different decimals", async () => {
        ...
        const aaveUsdValue = amountIn.mul(aavePrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));
        ...
        await mockUniswapRouter.setAmountOut(amountToSwap, aave.address, usdc.address, expectedUSDCAmount);

        const result = await uniswapLiquiditySwapAdapter.getAmountsOut(amountIn, aave.address, usdc.address);
    ...
    describe("getAmountsIn", () => {
      it("should return the estimated required amountIn for the asset swap", async () => {
        ...
        const ethUsdValue = amountToSwap.mul(wethPrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));
        const daiUsdValue = amountOut.mul(daiPrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));

        await mockUniswapRouter.setAmountIn(amountOut, weth.address, dai.address, amountIn);

        const result = await uniswapLiquiditySwapAdapter.getAmountsIn(amountOut, weth.address, dai.address);
      ...
      it("should work correctly with different decimals", async () => {
        ...
        const inPerOutPrice = amountOut.mul(parseEther("1")).mul(parseEther("1")).div(amountToSwap.mul("1000000")); // usdc 6 decimals

        const aaveUsdValue = amountToSwap.mul(aavePrice).div(parseEther("1")).mul(usdPrice).div(parseEther("1"));
        ...
        const result = await uniswapLiquiditySwapAdapter.getAmountsIn(amountOut, aave.address, usdc.address);
