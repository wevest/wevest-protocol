...
makeSuite("Mainnet Check list", (testEnv: TestEnv) => {
  ...
  it("Borrow stable WETH and Full Repay with ETH", async () => {
    ...
    await waitForTx(
      await wethGateway.connect(user.signer).repayETH(MAX_UINT_AMOUNT, "1", user.address, { value: repaySize }),
    );
  ...
  it("Borrow variable WETH and Full Repay with ETH", async () => {
    ...
    await waitForTx(
      await wethGateway.connect(user.signer).repayETH(partialPayment, "2", user.address, { value: partialPayment }),
    );
    ...
    await waitForTx(
      await wethGateway.connect(user.signer).repayETH(MAX_UINT_AMOUNT, "2", user.address, { value: repaySize }),
    );
  ...
  it("Borrow ETH via delegateApprove ETH and repays back", async () => {
    ...
    // Full Repay WETH loan with native ETH
    await waitForTx(
      await wethGateway.connect(user.signer).repayETH(MAX_UINT_AMOUNT, "2", user.address, { value: borrowSize.mul(2) }),
    );
    const debtBalanceAfterFullRepay = await varDebtToken.balanceOf(user.address);
    expect(debtBalanceAfterFullRepay).to.be.eq(zero);
  });
